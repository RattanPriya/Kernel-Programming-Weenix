...
sys_read(read_args_t *arg)
{
    ...
    retval=copy_from_user(&kern_args, arg, sizeof(kern_args));
	if(retval<0){
		dbg(DBG_ERROR,"SYS_READ: copy from user returned %d\n",retval);
		curthr->kt_errno=retval;
		return -1;
	}
	if(NULL==(temp_buff=page_alloc()))
	{
		dbg(DBG_ERROR,"SYS_READ: page_alloc returned null. ENOMEM\n");
		curthr->kt_errno=-ENOMEM;
		return -1;
	}
	read_bytes=do_read(kern_args.fd,temp_buff,kern_args.nbytes);
	if(read_bytes<0)
	{
		dbg(DBG_ERROR,"SYS_READ: do_read returned %d\n",read_bytes);
		curthr->kt_errno=read_bytes;
		return -1;
	}

	retval=copy_to_user(arg->buf,&temp_buff,read_bytes);
	if(retval<0){
		dbg(DBG_ERROR,"SYS_READ: copy to user returned %d\n",retval);
		curthr->kt_errno=retval;
		return -1;
	}

	page_free(temp_buff);
	dbg(DBG_PRINT|DBG_VM,"SYS_READ: Returning successfully\n");
	return read_bytes;
}
...
*/
	...
	retval = copy_from_user(&kern_args, arg, sizeof(kern_args));
	if (retval < 0) {
		dbg(DBG_ERROR, "SYS_WRITE: copy from user returned %d\n", retval);
		curthr->kt_errno = retval;
		return -1;
	}
	/*doubt about what to do with temp_buff and why is a buff is reqd in write*/
	if (NULL == (temp_buff = page_alloc())) {
		dbg(DBG_ERROR, "SYS_WRITE: page_alloc returned null. ENOMEM\n");
		curthr->kt_errno = -ENOMEM;
		return -1;
	}
	kern_args.buf=temp_buff; /*????*/
	write_bytes = do_write(kern_args.fd, kern_args.buf, kern_args.nbytes);
	if (write_bytes < 0) {
		dbg(DBG_ERROR, "SYS_WRITE: do_write returned %d\n", write_bytes);
		curthr->kt_errno = write_bytes;
		return -1;
	}

	retval = copy_to_user(arg->buf, &temp_buff, retval);
	if (retval < 0) {
		dbg(DBG_ERROR, "SYS_WRITE: copy to user returned %d\n", retval);
		curthr->kt_errno = retval;
		return -1;
	}

	page_free(temp_buff);
	dbg(DBG_PRINT|DBG_VM, "SYS_WRITE: Returning successfully\n");
	return write_bytes;
}
...
sys_getdents(getdents_args_t *arg)
{
    ...
    retval=copy_from_user(&kern_args, arg, sizeof(kern_args));
	if(retval<0){
		dbg(DBG_ERROR,"SYS_GETDENTS: copy from user returned %d\n",retval);
		curthr->kt_errno=retval;
		return -1;
	}

	if(NULL==(temp_buff=kmalloc(sizeof(dirent_t))))
	{
		dbg(DBG_ERROR,"SYS_GETDENTS: page_alloc returned null. ENOMEM\n");
		curthr->kt_errno=-ENOMEM;
		return -1;
	}
	kern_args.dirp=temp_buff;

	dir_count=0;
	while(dir_count<kern_args.count)
	{
		bytes_count=do_getdent(kern_args.fd,kern_args.dirp);
		if(bytes_count<0)
		{
			dbg(DBG_ERROR,"SYS_GETDENTS: do_getdent returned %d\n",bytes_count);
			curthr->kt_errno=bytes_count;
			return -1;
		}
		dir_count=dir_count+bytes_count;
	}
	retval=copy_to_user(arg,&kern_args,dir_count);
	if(retval<0){
		dbg(DBG_ERROR,"SYS_GETDENTS: Copy to user returned %d\n",retval);
		curthr->kt_errno=retval;
		return -1;
	}

	page_free(temp_buff);
	dbg(DBG_PRINT|DBG_VM,"SYS_GETDENTS: Returning successfully\n");
	return bytes_count;
}
