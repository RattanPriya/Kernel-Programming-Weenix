...
vmarea_t * find_vmarea(struct proc *p, const void *vaddr)
{
	vmarea_t *vma;

    list_iterate_begin(&p->p_vmmap->vmm_list, vma, vmarea_t, vma_plink) {
    	if((uint32_t)vaddr > vma->vma_start && (uint32_t)vaddr < vma->vma_end)
    	{
    		return vma;
    	}
    } list_iterate_end();
 ...
 * answering the question "does process p have permission perm on address vaddr?")
 */
...
*/
	int allowed=0;
	vmarea_t *vma;
	vma=find_vmarea(p,vaddr);
	if(vma!=NULL && (vma->vma_prot&perm)==perm)
		allowed= 1;
	dbg(DBG_PRINT | DBG_VM,
			"ADDR_PERM: Permission checked. Returning value = %d (0 means not allowed, 1 means allowed)\n",allowed);
	return allowed;
}

/*
 * range_perm is essentially a version of addr_perm that checks an entire
 * range of addresses (from avaddr to avaddr+len).
 * Though you will
...
*/
	...
	void *temp=0;
	vmarea_t *vma;
	while((uint32_t)temp<((uint32_t)avaddr)+len)
	{
		vma=find_vmarea(p,avaddr);
		if(vma==NULL)
			allowed=0;
		if(vma!=NULL && (vma->vma_prot&perm)!=perm)
			allowed=0;
	...
	dbg(DBG_PRINT | DBG_VM,
			"RANGE_PERM: Permission checked. Returning value = %d (0 means not allowed, 1 means allowed)\n",allowed);
	return allowed;
}
