...
vget(struct fs *fs, ino_t vno)
{
        ...
        dbg_print("vget: looking for inuse\n");
        KASSERT(fs);
        dbg_print("vget: looking for inuse vnode\n");
...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
		int retval;
		bytedev_t *temp;
		dbg(DBG_ERROR,"vnode.c: special_file_read: Entered special_file_read function.");
		KASSERT(file);
		...
		vget(file->vn_fs,file->vn_vno);
		if(S_ISBLK(file->vn_mode)||file->vn_bdev!=NULL )
		{
			dbg(DBG_ERROR,"vnode.c: special_file_read: Error! This is a block device.");
			vput(file);
			return -ENOTSUP;
		}

		if(file->vn_cdev!=NULL && file->vn_cdev == bytedev_lookup(file->vn_devid) )
		{
			temp=bytedev_lookup(file->vn_cdev->cd_id);
			retval=temp->cd_ops->read(temp,offset,buf,count);
			dbg(DBG_ERROR,"vnode.c: special_file_read: Read the special file. Returning with return value %d.",retval);
			vput(file);
			return retval;
		}
		vput(file);
		dbg_print("vnode.c: special_file_read: Error! Neither a block device nor a byte device.");
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
		...
		dbg_print("vnode.c: special_file_write: Entered special_file_read function.");
		vget(file->vn_fs,file->vn_vno);
		/*If the file is a block device....*/
		if(file->vn_bdev!=NULL)
		{
			dbg_print("vnode.c: special_file_write: Error! This is a block device.");
			vput(file);
			return -ENOTSUP;
		}

		/*If the file is a byte device device....*/
		if(file->vn_cdev!=NULL)
		{
			temp=bytedev_lookup(file->vn_cdev->cd_id);
			retval=temp->cd_ops->write(temp,offset,buf,count);
			dbg_print("vnode.c: special_file_write: Read the special file. Returning with return value %d.",retval);
			vput(file);
			return retval;
		}
		/*        NOT_YET_IMPLEMENTED("VFS: special_file_write");*/
		dbg_print("vnode.c: special_file_write: Error! Neither a block device nor a byte device.");
		/*This return should never be reached*/
		vput(file);
        return 0;
}
...
*/
	...
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_MMAP: KASSERT(file) executed successfully\n");

	KASSERT(S_ISCHR(file->vn_mode) && "because these ops only assigned if vnode represents a special file");
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_MMAP: KASSERT(S_ISCHR(file->vn_mode) executed successfully\n");

	KASSERT((file->vn_cdev) && "because open shouldn\'t have let us arrive here if vn_cdev was NULL");
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_MMAP: KASSERT((file->vn_cdev) executed successfully\n");
	KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->mmap);
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_MMAP: KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->mmap) executed successfully\n");

	retval=file->vn_ops->mmap(file,vma,ret);

	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_MMAP: Returning with return value %d \n", retval);
	return retval;
...
*/
	...
	KASSERT(file);
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_FILLPAGE: KASSERT(file) executed successfully\n");

	KASSERT(S_ISCHR(file->vn_mode));
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_FILLPAGE: KASSERT(S_ISCHR(file->vn_mode) executed successfully\n");

	KASSERT((file->vn_cdev));
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_FILLPAGE: KASSERT((file->vn_cdev) executed successfully\n");
	KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->fillpage);
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_FILLPAGE: KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->fillpage) executed successfully\n");

	retval=file->vn_ops->fillpage(file,offset,pagebuf);

	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_FILLPAGE: Returning with return value %d \n", retval);
	return retval;
}
...
*/
	...
	KASSERT(file);
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_DIRTYPAGE: KASSERT(file) executed successfully\n");

	KASSERT(S_ISCHR(file->vn_mode) && "because these ops only assigned if vnode represents a special file");
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_DIRTYPAGE: KASSERT(S_ISCHR(file->vn_mode) executed successfully\n");

	KASSERT((file->vn_cdev) && "because open shouldn\'t have let us arrive here if vn_cdev was NULL");
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_DIRTYPAGE: KASSERT((file->vn_cdev) executed successfully\n");
	KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->dirtypage);
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_DIRTYPAGE: KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->dirtypage) executed successfully\n");

	retval=file->vn_ops->dirtypage(file,offset);

	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_DIRTYPAGE: Returning with return value %d \n", retval);
	return retval;
...
*/
	...
	KASSERT(file);
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_CLEANPAGE: KASSERT(file) executed successfully\n");

	KASSERT(S_ISCHR(file->vn_mode) && "because these ops only assigned if vnode represents a special file");
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_CLEANPAGE: KASSERT(S_ISCHR(file->vn_mode) executed successfully\n");

	KASSERT((file->vn_cdev) && "because open shouldn\'t have let us arrive here if vn_cdev was NULL");
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_CLEANPAGE: KASSERT((file->vn_cdev) executed successfully\n");
	KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->cleanpage);
	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_CLEANPAGE: KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->cleanpage); executed successfully\n");

	retval=file->vn_ops->cleanpage(file,offset,pagebuf);

	dbg(DBG_PRINT | DBG_VM,
			"SPECIAL_FILE_CLEANPAGE: Returning with return value %d \n", retval);
	return retval;
